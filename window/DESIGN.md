...
## Project Override: Calendar Workspace

This product remains public-name-neutral. For `/dashboard`, the calendar workspace overrides marketing-page composition while retaining the warm cream canvas, editorial serif hierarchy, restrained coral action, low-shadow color blocks, and 8/12/16px radius scale above.

- Make the month grid and focused-day rundown the dominant first screen: dark calendar on the left, independently scrollable light detail panel on the right, then stack calendar before rundown below 1024px.
- Use the listed Smart Calendar palette (`#070B13`, `#072962`, `#0438A1`, `#124DDB`, `#D8DADD`, `#B58941`) for the product surface. Every event color must be paired with a symbol and written category/state.
- Keep projection evidence, placements, sources, privacy, and read-only import boundaries complete in clearly labeled native disclosures below the workspace.
- Name the prototype assistant **Pennyworth** and use the concise hierarchy “Ask Pennyworth”; assistant messages are labeled “Pennyworth,” while actions retain plain descriptive labels. Pennyworth remains an explicit deterministic local preview that may summarize a day, explain conflicts, reconfirm category colors, or preview focus protection; it must not imply provider connectivity, persistence, source mutation, or code execution.
- Preserve visible focus, reduced motion, semantic controls, 320px reflow, and 200% zoom without horizontal page overflow.

Visual direction comes from [Smart Calendar Dashboard by Bato](https://dribbble.com/shots/26543929-Smart-Calendar-Dashboard-UI-Design) for its clean month/detail split and priority color system, with [Agenda Calendar](https://dribbble.com/shots/5496606-Agenda-Calendar) as a secondary reference for a compact calendar-to-agenda relationship. These are layout references, not pixel-copy or branding sources.

Scheduling behavior is deliberately bounded to the high-value patterns in [Google Calendar focus time](https://support.google.com/calendar/answer/11190973) and [appointment availability](https://support.google.com/calendar/answer/10729749), [Microsoft Scheduling Assistant](https://support.microsoft.com/en-us/outlook/use-the-scheduling-assistant-and-room-finder-for-meetings-in-outlook), and Calendly's [buffers](https://help.calendly.com/hc/en-us/articles/14048208107287-How-to-use-buffers), [availability rules](https://help.calendly.com/hc/en-us/articles/1500004754122-Managing-additional-rules-for-your-availability), and [reusable event types](https://help.calendly.com/hc/en-us/articles/4914418007831). Do not extend this into routing forms, round-robin assignment, enterprise administration, CRM, or notification workflows.
