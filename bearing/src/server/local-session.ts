...
const NATIVE_HTML_TEMPLATE =
  ...
  '  function resumeRailEntry(entry) { [repositoryPanel, routeForm, workForm, planningPanel, planReviewPanel, historyPanel, demoPanel].forEach(function (panel) { panel.hidden = true; }); var resumable = !!(entry.stage && entry.lastResult && entry.status !== "complete"); if (entry.busy && entry.stage) { currentRunId = entry.runId; currentGoal = entry.goal; currentStage = entry.stage; planningPanel.hidden = false; showWait(currentStage); setStatus("Returned to the active journey.", true); } else if (resumable) { currentRunId = entry.runId; currentGoal = entry.goal; currentStage = entry.stage; planningPanel.hidden = false; renderJourney(Object.assign({}, entry.lastResult, { artifacts: entry.artifacts || [], artifactLinks: entry.artifactLinks || [] })); setStatus("Resumed the saved journey.", false); } else { currentRunId = ""; currentGoal = ""; workForm.hidden = false; document.getElementById("work-goal").value = entry.goal; document.getElementById("work-goal").focus(); setStatus("Saved request loaded. Embark when you are ready to start a new journey.", false); } renderRailHistory(historyEntries); appShell.classList.remove("rail-open"); }\n' +
