...
const NATIVE_HTML_TEMPLATE =
  ...
  '  var waitRanges = { "set-bearings": { label: "about 3 minutes", max: 180 }, "gather-supplies": { label: "5\u201360 minutes", max: 3600 }, "map-route": { label: "10\u201335 minutes \u2014 design, validation, and implementation slicing", max: 2100 }, "draft-implementation": { label: "about 5 minutes", max: 300 }, "execute-explorer": { label: "varies\u2014minutes to hours", max: 0 }, "execute-expedition": { label: "varies\u2014often hours", max: 0 }, "review": { label: "varies with the size of the diff", max: 0 } };\n' +
  ...
  '  function updateWaitClock() { var elapsed = Math.floor((Date.now() - elapsedStarted) / 1000); var activity = waitActivityAt ? Math.max(0, Math.floor((Date.now() - waitActivityAt) / 1000)) : null; var range = waitRanges[currentStage]; document.getElementById("wait-elapsed").textContent = elapsed + "s elapsed"; document.getElementById("wait-activity").textContent = "Last real activity: " + (activity === null ? "waiting for the first event." : activity < 2 ? "just now." : activity + "s ago."); document.getElementById("wait-slow").hidden = !range || !range.max || elapsed <= range.max; }\n' +
  ...
  '  function showWait(stage) { ensureWaitControls(); setStatus(phaseNames[stage] + " is working\u2026", true); document.getElementById("journey-content").hidden = true; document.getElementById("journey-wait").hidden = false; document.getElementById("git-change-panel").hidden = true; document.getElementById("git-count").setAttribute("aria-expanded", "false"); document.getElementById("journey-body").setAttribute("aria-busy", "true"); document.getElementById("wait-phase").textContent = phaseNames[stage]; document.getElementById("wait-help").textContent = waitExplanations[stage]; document.getElementById("wait-status").textContent = "Waiting for real activity\u2026"; document.getElementById("wait-range").textContent = "Typical time: " + waitRanges[stage].label; document.getElementById("wait-slow").hidden = true; document.getElementById("trail-log").replaceChildren(); document.getElementById("steer-journey").disabled = false; document.getElementById("stop-journey").disabled = false; elapsedStarted = Date.now(); waitActivityAt = 0; waitActivitySequence = 0; waitSignature = ""; clearInterval(elapsedTimer); clearInterval(statusTimer); updateWaitClock(); elapsedTimer = setInterval(updateWaitClock, 1000); refreshJourneyStatus(); statusTimer = setInterval(refreshJourneyStatus, 2000); }\n' +
  ...
  '  function loadHistory() { setStatus("Loading local journey history\u2026", true); fetch("/api/v1/history", { credentials: "same-origin" }).then(function (r) { if (!r.ok) requestError("History could not be loaded", r); return r.json(); }).then(function (body) { renderHistory(body.history || []); setStatus("Journey history for this repository.", false); }, showError); }\n' +
