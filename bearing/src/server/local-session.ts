...
const NATIVE_HTML =
  ...
  '<style>.model-note{display:flex;align-items:center;justify-content:space-between;gap:12px;color:var(--muted);margin:0 0 12px}.model-note code{color:var(--ink)}.wait-controls{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;margin-top:14px}.wait-controls input{min-width:0}.danger{border-color:#9b3c3c;color:#ffd7d7}.git-count{white-space:nowrap}.history-list{display:grid;gap:10px}.history-card{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:center;border:1px solid var(--line2);border-radius:9px;background:rgba(15,16,17,.45);padding:14px}.history-card strong,.history-card span{display:block}.history-card span{color:var(--subtle);font-size:12px}.nav-action{min-height:32px;padding:3px 9px;border-radius:999px;background:var(--s1);color:var(--muted);font-size:12px}@media(max-width:760px){.wait-controls{grid-template-columns:1fr 1fr}.wait-controls input{grid-column:1/-1}.history-card{grid-template-columns:1fr}}</style>\n' +
  ...
  '<form class="panel" id="route-form" hidden><div class="panel-head"><h2>Agent harness</h2><span class="step">02 / LAUNCH</span></div><div class="panel-body"><fieldset class="route-fieldset"><legend>Choose an installed agent</legend><p class="model-note"><span>Bearing uses that agent\u2019s current <code>/model</code> selection. Change the model or reasoning in the agent, then refresh.</span><button id="refresh-routes" type="button">Refresh</button></p><p id="detected-routes">Checking agent and model settings\u2026</p><div class="route-options" id="route-options"></div></fieldset><div class="route-details"><div><label for="owner-name">What should we call you?</label><input id="owner-name" type="text" required autocomplete="name" maxlength="80"></div></div><div class="actions actions-end"><button class="primary" id="launch-bearing" disabled>Launch</button></div></div></form>\n' +
  ...
  '  function ensureWaitControls() { if (document.getElementById("wait-controls")) return; var meta = document.querySelector(".wait-meta"); var git = document.createElement("span"); git.id = "git-count"; git.className = "git-count"; git.textContent = "Git: checking\u2026"; meta.insertBefore(git, document.getElementById("wait-elapsed")); var controls = document.createElement("div"); controls.id = "wait-controls"; controls.className = "wait-controls"; var input = document.createElement("input"); input.id = "steer-instruction"; input.maxLength = 4096; input.placeholder = "Steer this phase\u2026"; input.setAttribute("aria-label", "Steering instruction"); var steer = document.createElement("button"); steer.id = "steer-journey"; steer.type = "button"; steer.textContent = "Steer"; var stop = document.createElement("button"); stop.id = "stop-journey"; stop.type = "button"; stop.className = "danger"; stop.textContent = "Stop"; controls.append(input, steer, stop); document.getElementById("journey-wait").insertBefore(controls, document.getElementById("artifact-checklist")); steer.addEventListener("click", function () { sendJourneyControl("steer"); }); stop.addEventListener("click", function () { sendJourneyControl("stop"); }); }\n' +
  '  function refreshJourneyStatus() { if (!currentRunId) return; fetch("/api/v1/journey/" + encodeURIComponent(currentRunId) + "/status", { credentials: "same-origin" }).then(function (r) { if (!r.ok) return null; return r.json(); }).then(function (body) { if (!body) return; document.getElementById("git-count").textContent = body.changedFiles === null ? "Git unavailable" : "Git: " + body.changedFiles + " changed " + (body.changedFiles === 1 ? "file" : "files"); if (body.run) renderArtifacts(body.run); }); }\n' +
  '  function sendJourneyControl(action) { var input = document.getElementById("steer-instruction"); var instruction = input.value.trim(); if (action === "steer" && !instruction) { input.focus(); return; } document.getElementById("steer-journey").disabled = true; document.getElementById("stop-journey").disabled = true; fetch("/api/v1/journey/control", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin", body: JSON.stringify({ runId: currentRunId, action: action, ...(action === "steer" ? { instruction: instruction } : {}) }) }).then(function (r) { if (!r.ok) requestError("Journey control was rejected", r); input.value = ""; document.getElementById("wait-status").textContent = action === "steer" ? "Steering received. Restarting this phase\u2026" : "Stopping the active agent\u2026"; }, function () { document.getElementById("steer-journey").disabled = false; document.getElementById("stop-journey").disabled = false; setStatus("Journey control failed. Try again.", false); }); }\n' +
  '  function showWait(stage) { ensureWaitControls(); setStatus(phaseNames[stage] + " is working\u2026", true); document.getElementById("journey-content").hidden = true; document.getElementById("journey-wait").hidden = false; document.getElementById("journey-body").setAttribute("aria-busy", "true"); document.getElementById("wait-phase").textContent = phaseNames[stage]; document.getElementById("wait-status").textContent = "Waiting for the selected agent\u2026"; document.getElementById("steer-journey").disabled = false; document.getElementById("stop-journey").disabled = false; elapsedStarted = Date.now(); document.getElementById("wait-elapsed").textContent = "0s elapsed"; clearInterval(elapsedTimer); clearInterval(statusTimer); elapsedTimer = setInterval(function () { document.getElementById("wait-elapsed").textContent = Math.floor((Date.now() - elapsedStarted) / 1000) + "s elapsed"; }, 1000); refreshJourneyStatus(); statusTimer = setInterval(refreshJourneyStatus, 2000); }\n' +
  ...
  '  function renderRoutes(routes) { var names = { "codex": "Codex CLI", "claude": "Claude Code", "grok-safe": "Grok Safe", "pi": "Pi" }; var firstAvailable = null; selectedRoute = null; document.getElementById("launch-bearing").disabled = true; routeOptions.replaceChildren(); routes.forEach(function (route, index) { var label = document.createElement("label"); label.className = "route-card" + (route.detected ? "" : " unavailable"); var input = document.createElement("input"); input.type = "radio"; input.name = "route"; input.id = "route-option-" + index; input.required = true; input.disabled = !route.detected; input.addEventListener("change", function () { selectedRoute = route; document.getElementById("launch-bearing").disabled = false; }); var copy = document.createElement("span"); var title = document.createElement("strong"); title.textContent = names[route.id] || route.id; var statusText = document.createElement("span"); statusText.className = "route-status"; statusText.id = input.id + "-status"; statusText.textContent = route.detected ? "Agent detected" : "Agent unavailable"; input.setAttribute("aria-describedby", statusText.id); var modelText = document.createElement("span"); modelText.className = "route-model"; modelText.textContent = (route.model === "*" ? "Current model: agent default" : "Current model: " + route.model) + " · reasoning: " + route.reasoning; copy.append(title, statusText, modelText); label.append(input, copy); routeOptions.appendChild(label); if (!firstAvailable && route.detected) firstAvailable = input; }); var detected = routes.filter(function (route) { return route.detected; }).length; detectedRoutes.textContent = detected + " of " + routes.length + " supported agents detected. Choose one to enable Launch; Bearing will verify it before starting."; if (firstAvailable) firstAvailable.focus(); }\n' +
...
function gitChangedFiles(repositoryPath: string): Promise<number | null> {
  return new Promise((resolveCount) => execFile("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], { cwd: repositoryPath, encoding: "utf8", timeout: 3_000, maxBuffer: 1024 * 1024 }, (error, stdout) => {
    if (error) { resolveCount(null); return; }
    const records = stdout.split("\0").filter(Boolean);
    let count = 0;
    for (let index = 0; index < records.length; index += 1) {
      count += 1;
      if (/[RC]/.test(records[index].slice(0, 2))) index += 1;
    }
    resolveCount(count);
  }));
}
...
async function handleJourneyStatusGet(req: IncomingMessage, res: ServerResponse, service: LocalSessionService, selected: SelectedBrowserState, runId?: string): Promise<void> {
  ...
  const changedFiles = await gitChangedFiles(selected.repositoryPath);
  ...
  writeShowcaseJson(res, { changedFiles, history: boundedHistory, ...(state ? { run: { runId, goal: state.goal, stage: state.stage, status: state.status, busy: state.busy, artifacts: state.artifacts, question: state.question, lastResult: state.lastResult } } : {}) });
}
