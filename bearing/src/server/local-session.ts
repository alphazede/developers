...
const NATIVE_HTML =
  ...
  '<style>.repo-switch{min-height:32px;padding:3px 9px;border-radius:999px;background:var(--s1);color:var(--muted);font-size:12px}.panel:not([hidden]){animation:panel-in .3s cubic-bezier(.2,.8,.2,1) both}.status.busy::before{content:"";display:inline-block;width:9px;height:9px;margin-right:9px;border:2px solid var(--accent);vertical-align:-1px;animation:compass-spin .8s linear infinite}.prompt-panel textarea{min-height:132px;font-size:16px}.prompt-panel .hint{margin:0;color:var(--subtle);font-size:12px}.prompt-panel .actions{justify-content:space-between}@keyframes panel-in{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}@keyframes compass-spin{to{transform:rotate(360deg)}}@media(max-width:760px){.local-badge{display:none}.repo-switch{max-width:145px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.prompt-panel .actions{align-items:stretch;flex-direction:column}}</style>\n' +
  "</head>\n" +
  "<body>\n" +
  ...
  '<form class="panel prompt-panel" id="work-form" hidden><div class="panel-head"><h2>What are we working on?</h2><span class="step">03 / ASK</span></div><div class="panel-body"><label for="work-goal">Describe the work</label><textarea id="work-goal" required maxlength="4096" placeholder="Build, fix, investigate, or prepare something..."></textarea><div class="actions"><span class="hint">Bearing starts lean with Explorer and shows Expedition when the work needs more coordination.</span><button class="primary">Set direction</button></div></div></form>\n' +
  '<section class="panel" id="recommendation" hidden aria-live="polite"><div class="panel-head"><h2>Suggested crew</h2><span class="step">ONE CHOICE</span></div><div class="panel-body"><p id="recommended-mode"></p><p id="token-warning"></p><p id="token-tradeoff"></p><p id="coordination-tradeoff"></p><p id="approval-state"></p><div class="actions" id="recommendation-actions" hidden><button class="primary" id="approve-mode" type="button">Use suggested route</button><button id="override-mode" type="button">Use alternate route</button></div></div></section>\n' +
  ...
  '  function showRun(state) { var r = state.recommendation; runState = state; if (!r) return; recommendation.hidden = false; document.getElementById("recommended-mode").textContent = "Bearing suggests " + r.recommendedMode + " (" + r.estimatedAgents + " agents)."; document.getElementById("token-warning").textContent = "Estimated token use: " + r.estimatedTokens + "."; document.getElementById("token-tradeoff").textContent = "Token tradeoff: " + r.tradeoffs.tokens; document.getElementById("coordination-tradeoff").textContent = "Coordination tradeoff: " + r.tradeoffs.coordination; document.getElementById("approval-state").textContent = state.approval ? "Route selected: " + state.approval.selectedMode + ". No work was launched." : "Confirm the suggestion or use the alternate route."; recommendationActions.hidden = !!state.approval; }\n' +
  ...
  '  function revealWork(greeting) { onboardingReady = true; repositoryPanel.hidden = true; routeForm.hidden = true; workForm.hidden = false; changeRepository.hidden = false; changeRepository.textContent = "Change repository"; setStatus(greeting, false); document.getElementById("work-goal").focus(); }\n' +
  ...
  '  function toggleRepositoryChooser() { if (!repositoryPanel.hidden) { repositoryPanel.hidden = true; changeRepository.textContent = "Change repository"; if (returnPanel) returnPanel.hidden = false; if (returnPanel === workForm && runState && runState.recommendation) showRun(runState); setStatus(rememberedGreeting || "Repository unchanged.", false); return; } returnPanel = workForm.hidden ? routeForm : workForm; routeForm.hidden = true; workForm.hidden = true; recommendation.hidden = true; repositoryPanel.hidden = false; changeRepository.textContent = "Keep current"; setStatus("Choose another repository.", false); loadRepositoryOptions(); }\n' +
  ...
  "    ev.preventDefault();\n" +
  '    var ownerName = document.getElementById("owner-name"); var name = ownerName.value.trim(); if (!name) { ownerName.setCustomValidity("Tell us what to call you."); ownerName.reportValidity(); return; } ownerName.setCustomValidity(""); if (!routeForm.reportValidity() || !selectedRoute) return; setStatus("Checking the route...", true); fetch("/api/v1/readiness", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin", body: JSON.stringify({ provider: selectedRoute.provider, model: selectedRoute.model, reasoning: document.getElementById("reasoning").value }) }).then(function (r) { return r.json(); }).then(function (body) { setStatus(body.status === "detected" ? "CLI detected; provider verification required." : body.status === "blocked" ? "Route unavailable." : status.textContent, body.status === "detected"); if (body.status === "ready") finishReady(name, false); }, function () { setStatus("Readiness check failed.", false); });\n' +
  "  });\n" +
  '  workForm.addEventListener("submit", function (ev) {\n' +
  '    ev.preventDefault(); if (!workForm.reportValidity()) return; var goal = document.getElementById("work-goal").value.trim(); if (!goal) return; currentRunId = "browser-" + crypto.randomUUID(); runState = null; recommendation.hidden = true; var values = { workItems: 1, maxCrewmatesPerExplorer: 3, perAgentTokenEstimate: 4000 }; setStatus("Mapping the leanest route...", true); readRun(currentRunId).then(function (state) { if (state.workRequestCreated) return state; return postCommand(currentRunId, state, "createWorkRequest", { title: goal.split(/\\r?\\n/, 1)[0].slice(0, 160), goal: goal }).then(function () { return readRun(currentRunId); }); }).then(function (state) { if (state.recommendation) return state; return postCommand(currentRunId, state, "recommendExecutionMode", values).then(function () { return readRun(currentRunId); }); }).then(function (state) { showRun(state); setStatus(state.recommendation ? "Route suggestion ready." : "Recommendation unavailable.", false); }, showError);\n' +
  "  });\n" +
  '  document.getElementById("approve-mode").addEventListener("click", function () { if (!runState || !runState.recommendation || runState.approval) return; setStatus("Saving your route...", true); postCommand(runId(), runState, "approveExecutionMode", { recommendationEventId: runState.recommendation.eventId }).then(function () { return readRun(runId()); }).then(function (state) { showRun(state); setStatus("Suggested route saved. No work was launched.", false); }, showError); });\n' +
  '  document.getElementById("override-mode").addEventListener("click", function () { if (!runState || !runState.recommendation || runState.approval) return; var alternate = runState.recommendation.recommendedMode === "explorer" ? "expedition" : "explorer"; setStatus("Saving the alternate route...", true); postCommand(runId(), runState, "overrideExecutionMode", { recommendationEventId: runState.recommendation.eventId, selectedMode: alternate }).then(function () { return readRun(runId()); }).then(function (state) { showRun(state); setStatus("Alternate route saved. No work was launched.", false); }, showError); });\n' +
  "})();\n" +
  "</script>\n" +
  "</body>\n" +
  "</html>\n";
