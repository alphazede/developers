...
const NATIVE_HTML =
  ...
  "</head>\n" +
  "<body>\n" +
  '<header><div class="brand"><span class="brand-mark" aria-hidden="true"></span>Bearing <button class="demo-link" id="view-demo" type="button" hidden>Live demo</button></div><nav class="nav-state" aria-label="Runtime status"><button class="repo-switch" id="change-repository" type="button" hidden>Change repository</button><span class="badge local-badge">LOCAL</span><span class="badge">OWNER CONTROLLED</span></nav></header>\n' +
  "<main>\n" +
  ...
  '<section class="panel" id="planning-panel" hidden aria-live="polite"><div class="panel-head"><h2>Planning handoff ready</h2><span class="step">PLAN FIRST</span></div><div class="panel-body"><p>Your work request is saved. Bearing will ask how to execute only after <code>implementation.md</code> is ready for your review.</p><p>No execution mode was selected and no agent work was launched.</p><p>Would you like a live demo of the complete workflow?</p><div class="actions actions-end"><button class="primary" id="planning-demo" type="button">Start live demo</button></div></div></section>\n' +
  '<section class="panel demo-panel" id="demo-panel" hidden aria-labelledby="demo-heading"><div class="panel-head"><h2 id="demo-heading">How Bearing works</h2><div class="panel-head-actions"><span class="step">NO TOKENS</span><button class="compact-back" id="close-demo" type="button">Close</button></div></div><div class="panel-body"><div class="demo-progress" aria-label="Demo progress"><span class="active">1 Request</span><span>2 Plan</span><span>3 Choose</span><span>4 Execute</span><span>5 Survey</span></div><div class="demo-stage" data-demo-stage="0"><h3>Tell Bearing what needs to change</h3><p>Start with the outcome, repository, and any reference material. Bearing asks planning questions before choosing an execution shape.</p><div class="demo-example">Example: Add bulk customer onboarding with validation, duplicate handling, a dry-run preview, audit evidence, tests, and an independent review.</div></div><div class="demo-stage" data-demo-stage="1" hidden><h3>Review the implementation plan</h3><p>Bearing maps requirements, design, validation, and bounded implementation slices. The execution decision remains locked until <code>implementation.md</code> is ready.</p><div class="demo-example"><strong>implementation.md ready</strong><br>5 bounded slices \u00b7 2 parallel lanes \u00b7 security and reliability gates identified</div></div><div class="demo-stage" data-demo-stage="2" hidden><h3>Choose how to execute the approved plan</h3><p>The owner chooses. More bears mean more parallel work, coordination, and token use.</p><div class="mode-grid"><button class="mode-card" id="demo-explorer" type="button" aria-pressed="false"><img src="/assets/bearing-explorer-card.png" alt="Two bears following one focused mountain route."><span class="mode-copy"><strong>Explorer</strong><span class="mode-kicker">Focused route \u00b7 fewer agent sessions</span><span class="mode-line"><b>Use when:</b> the plan is compact, mostly sequential, or has one clear workstream.</span><span class="mode-line"><b>Pros:</b> lower token use and simpler coordination.</span><span class="mode-line"><b>Tradeoff:</b> less parallelism on broad plans.</span></span></button><button class="mode-card" id="demo-expedition" type="button" aria-pressed="false"><img src="/assets/bearing-expedition-card.png" alt="Five bears coordinating multiple mountain activities."><span class="mode-copy"><strong>Expedition</strong><span class="mode-kicker">Parallel ascent \u00b7 more agent sessions</span><span class="mode-line"><b>Use when:</b> the plan has several independent lanes, specialties, or waves.</span><span class="mode-line"><b>Pros:</b> more parallel progress and dedicated coordination.</span><span class="mode-line"><b>Tradeoff:</b> higher token use and coordination overhead.</span></span></button></div><p id="demo-mode-status" role="status">Choose a card to continue the demo.</p></div><div class="demo-stage" data-demo-stage="3" hidden><h3>Execute only the approved slices</h3><p id="demo-selected-mode">Your selected execution mode appears here.</p><p>Agents follow the approved implementation contract, stop at owner decisions, and record evidence instead of silently expanding scope.</p></div><div class="demo-stage" data-demo-stage="4" hidden><h3>Survey the result independently</h3><p>A Surveyor reviews outcomes and evidence outside the execution chain. Findings route back for owner-approved remediation and Resurvey.</p><div class="demo-example">Completed \u00b7 Deviated \u00b7 Blocked \u00b7 Not performed \u2014 each backed by evidence.</div></div><div class="demo-actions"><button id="demo-prev" type="button" disabled>\u2190 Previous</button><button class="primary" id="demo-next" type="button">Next \u2192</button></div></div></section>\n' +
  ...
  '  function showDemoStage(next) { var stages = document.querySelectorAll("[data-demo-stage]"); var progress = document.querySelectorAll(".demo-progress span"); demoStage = Math.max(0, Math.min(stages.length - 1, next)); stages.forEach(function (stage, index) { stage.hidden = index !== demoStage; }); progress.forEach(function (step, index) { step.classList.toggle("active", index === demoStage); }); document.getElementById("demo-prev").disabled = demoStage === 0; document.getElementById("demo-next").textContent = demoStage === stages.length - 1 ? "Restart demo" : "Next \\u2192"; }\n' +
  '  function chooseDemoMode(mode) { demoMode = mode; ["explorer", "expedition"].forEach(function (name) { var card = document.getElementById("demo-" + name); var selected = name === mode; card.classList.toggle("selected", selected); card.setAttribute("aria-pressed", String(selected)); }); if (!mode) { document.getElementById("demo-mode-status").textContent = "Choose a card to continue the demo."; document.getElementById("demo-selected-mode").textContent = "Your selected execution mode appears here."; return; } document.getElementById("demo-mode-status").textContent = (mode === "explorer" ? "Explorer selected: focused execution with fewer agent sessions." : "Expedition selected: parallel execution with more coordination.") + " Demo only; nothing was launched."; document.getElementById("demo-selected-mode").textContent = "Demo selection: " + (mode === "explorer" ? "Explorer" : "Expedition") + ". In a real run, Bearing records owner approval before execution."; }\n' +
  '  function openDemo() { demoReturnPanel = !planningPanel.hidden ? planningPanel : !workForm.hidden ? workForm : !routeForm.hidden ? routeForm : repositoryPanel; [repositoryPanel, routeForm, workForm, planningPanel].forEach(function (panel) { panel.hidden = true; }); demoPanel.hidden = false; viewDemo.textContent = "Exit demo"; chooseDemoMode(""); showDemoStage(0); setStatus("Interactive demo. No model calls or tokens.", false); }\n' +
  '  function closeDemoPanel() { demoPanel.hidden = true; viewDemo.textContent = "Live demo"; if (demoReturnPanel) demoReturnPanel.hidden = false; setStatus(rememberedGreeting || "Demo closed.", false); }\n' +
  ...
  '  function revealWork(greeting) { onboardingReady = true; repositoryPanel.hidden = true; routeForm.hidden = true; planningPanel.hidden = true; workForm.hidden = false; changeRepository.hidden = false; viewDemo.hidden = false; changeRepository.textContent = "Change repository"; setStatus(greeting, false); document.getElementById("work-goal").focus(); }\n' +
  ...
  '  document.getElementById("planning-demo").addEventListener("click", openDemo);\n' +
  ...
  '  document.getElementById("demo-next").addEventListener("click", function () { if (demoStage === 2 && !demoMode) { document.getElementById("demo-mode-status").textContent = "Choose Explorer or Expedition before continuing."; return; } if (demoStage === 4) { demoMode = ""; chooseDemoMode(""); showDemoStage(0); return; } showDemoStage(demoStage + 1); });\n' +
