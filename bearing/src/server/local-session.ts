...
const NATIVE_HTML_TEMPLATE =
  ...
  '<header><div class="brand"><button class="rail-toggle" id="toggle-rail" type="button" aria-label="Open journey history">☰</button><img class="title-mark" src="/assets/bearing-title-mark.png" alt="">Bearing</div><nav class="nav-state" aria-label="Runtime status"><a class="repo-switch" href="https://github.com/alphazede/developers/tree/main/bearing" target="_blank" rel="noopener noreferrer">GitHub repo \u2197</a><button class="nav-action" id="history-button" type="button" hidden>History</button><button class="repo-switch" id="change-repository" type="button" hidden>Change repository</button><span class="badge local-badge">LOCAL</span><span class="badge">OWNER CONTROLLED</span></nav></header>\n' +
  ...
  '  function renderRecoveryReport(recovery) { var panel = document.getElementById("recovery-report"); document.getElementById("journey-summary").after(panel); panel.hidden = !recovery || (recovery.status !== "repaired" && recovery.status !== "stopped"); if (panel.hidden) return; document.getElementById("recovery-heading").textContent = recovery.status === "repaired" ? "Bearing repaired a recoverable agent error." : "Bearing stopped a repeated recoverable agent error."; document.getElementById("recovery-summary").textContent = "Stage " + recovery.stage + (recovery.status === "repaired" ? " recovered from " : " stopped after ") + recovery.code + " using " + recovery.retryLevel + "."; document.getElementById("report-recovery-bug").onclick = function () { var title = "Bearing " + recovery.version + ": " + recovery.code + " during " + recovery.stage; var body = ["Bearing version: " + recovery.version, "Stage: " + recovery.stage, "Failure class: " + recovery.failureClass, "Failure code: " + recovery.code, "Retry level: " + recovery.retryLevel].join("\\n"); window.open("https://github.com/alphazede/developers/issues/new?title=" + encodeURIComponent(title) + "&body=" + encodeURIComponent(body), "_blank", "noopener,noreferrer"); }; }\n' +
  ...
  '  document.getElementById("journey-retry").addEventListener("click", function () { document.getElementById("journey-complete").firstElementChild.textContent = "Evidence complete"; var stage = retryStage || currentStage; retryStage = ""; invokeJourney(stage); });\n' +
...
const NATIVE_HTML = NATIVE_HTML_TEMPLATE
  .replace("Your selected agent is working. Bearing will show only validated results.", "Bearing is creating or resuming the local plan stub and bounded repository map. Next: Gather Supplies discovers owner decisions.")
...
function isJourneyBody(v: unknown): v is JourneyBody {
  if (typeof v !== "object" || v === null || Array.isArray(v)) return false;
  const body = v as Record<string, unknown>;
  const allowed = new Set(["runId", "stage", "workGoal", "answer", "endQuestions", "reviewChange", "executionMode", "reviewCadence", "cleanupMergedWorktrees"]);
...
function recoverableFailure(result: JourneyResult): result is FailureResult {
  return result.status === "failure" && ["result_missing", "result_malformed", "artifact_invalid"].includes(result.code);
}

function recoveryFingerprint(stage: JourneyStage, result: FailureResult, state: BrowserJourney): string {
  return [stage, result.code, state.planDirectory ? "plan" : "no-plan", state.artifacts.length, state.qa.length, state.gatherQuestionsDiscovered ? "gathered" : "discovering"].join(":");
}

function clearRecovery(state: BrowserJourney): void { state.recovery = undefined; }
...
function handleJourneyPost(req: IncomingMessage, res: ServerResponse, service: LocalSessionService, selected: SelectedBrowserState, journey: JourneyService | undefined): void {
  ...
  readJsonBody(req, MAX_JOURNEY_BODY).then(async (value) => {
    ...
    if (value.stage === "execute-explorer" || value.stage === "execute-expedition") {
      ...
      if ((state.stage !== "map-route" && state.stage !== "draft-implementation" && state.stage !== value.stage) || state.reviewBaselineRevision === undefined || mode !== expectedMode || !["slice", "phase", "end"].includes(cadence ?? "")) { writeRejection(res, 409); return; }
      if (!planningApprovalRecorded(durable, state.reviewBaselineRevision)) { writeRejection(res, 409); return; }
    }
    const stageChanged = state.stage !== value.stage;
    const ownerSteered = Boolean(value.answer || value.reviewChange || value.executionMode || value.reviewCadence || value.cleanupMergedWorktrees !== undefined);
    ...
    try {
      ...
      let firstFailure: FailureResult["code"] | undefined;
      let lastRetryLevel: RecoveryReport["retryLevel"] = "repair";
      while (recoverableFailure(result)) {
        firstFailure ??= result.code;
        const fingerprint = recoveryFingerprint(value.stage, result, state);
        state.recovery = state.recovery?.fingerprint === fingerprint ? { fingerprint, repeats: state.recovery.repeats + 1 } : { fingerprint, repeats: 1 };
        if (state.recovery.repeats >= 3 || automaticRetries >= 2) break;
        lastRetryLevel = state.recovery.repeats === 1 ? "repair" : "simplify";
        automaticRetries += 1;
    ...
    const links = state.artifacts.flatMap((path, index) => /\.(?:html|md)$/i.test(path) ? [{ path, url: `/api/v1/journey/${encodeURIComponent(value.runId)}/artifacts/${index}` }] : []);
    writeShowcaseJson(res, { ...result, artifacts: state.artifacts, artifactLinks: links, ...(recoveryReport ? { recovery: recoveryReport } : {}) });
  }, (error: unknown) => writeRejection(res, error instanceof RangeError ? 413 : 400));
}
...
async function handleJourneyStatusGet(req: IncomingMessage, res: ServerResponse, service: LocalSessionService, selected: SelectedBrowserState, runId?: string, journey?: JourneyService): Promise<void> {
  ...
  writeShowcaseJson(res, { changedFiles, gitChanges: changes ?? [], history: boundedHistory, ...(runId ? { activityTrail: journey?.activityTrail(runId) ?? [] } : {}), ...(state ? { run: { runId, goal: state.goal, stage: state.stage, status: state.status, busy: state.busy, artifacts: state.artifacts, question: state.question, lastResult: state.lastResult } } : {}) });
}
