---
type: design-system
title: Bearing Design System
okf_status: active
tags:
  - developer
  - public
version: alpha
name: Bearing-design-system
description: "A near-black local control room built around #010102, light gray text, and one restrained lavender-blue accent. The system reads as precise expedition software: dense, technical, calm, and quietly luxurious. Native system fonts keep it portable. Cards live as charcoal panels with hairline borders; the accent appears on focus, active state, and primary actions rather than as decoration."
...
This document began with the `linear.app` profile from getdesign.md. Bearing
uses that profile as visual-system research only; it does not copy Linear
logos, product marks, proprietary fonts, screenshots, or interface content.

For Bearing's browser control room:

- Treat the active repository, configured crew, workflow, evidence, and owner
  approval as the product UI that should dominate the page.
- Use forest and navigation language sparingly: `Set Bearings`, `Navigator`,
  `Explorer`, `Crewmates`, and `Scouts`. Do not turn the interface into a
  decorative camping theme.
- Use the system font stack and `ui-monospace`; do not download external fonts.
- Keep lavender-blue as the sole interactive accent. Semantic success, warning,
  and failure colors may appear only when they communicate real state.
- Prefer compact cards, hairline dividers, strong alignment, and clear status
  badges. Avoid gradients, glass effects, oversized marketing heroes, and
  ornamental illustrations.
- Use the owner-provided bear-at-work photograph as Bearing's one signature
  visual. Keep it inside a framed brand panel with meaningful alt text; never
  place controls or essential text over the image, and never use it as a
  full-page background.
- Use the owner-approved multi-bear alpine illustration as the responsive page
  background. Preserve its dark left-side content-safe area, add sufficient
  overlay for contrast, and treat it as decorative; the live interface remains
  the accessible source of workflow and role meaning.
- Preserve keyboard focus, reduced-motion behavior, readable contrast, and
  touch targets of at least 40px (44px on narrow/touch layouts).
