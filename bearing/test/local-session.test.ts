...
describe("GET / native page and fragment secrecy", () => {
  it("serves the native page and never embeds the capability server-side", async () => {
    ...
    expect(r.body).toContain('Codex — configured model');
    expect(r.body).toContain('Grok Build');
    expect(r.body).toContain('GLM 5.2');
    expect(r.body).toContain('DeepSeek V4 Pro');
    expect(r.body).toContain('statusText.textContent = route.detected ? "CLI detected" : "CLI unavailable"');
    ...
    expect(r.body).toContain('JSON.stringify({ provider: selectedRoute.provider, model: selectedRoute.model, reasoning: document.getElementById("reasoning").value })');
